...
namespace RED
{
	...
	void PMT_R11410::SetDefaults() {        
		...
		fShape.func->SetParameter(1, 0);    //Center
		fShape.func->SetParameter(2, 1*ns); //Sigma
	...
	void PMT_R11410::SetParams (double QE, double Area_mean, double DCR, double AP_cont) {
		...
		cout << "QE=" << fQE << endl;
		cout << "SPE Area=" << fArea_mean << endl;
		cout << "DCR=" << fDCR << endl;
		cout << "AP=" << fAP_cont << endl;
		CalculateParams();
	}

	void PMT_R11410::SetDPE (Double_t DPE) {
		fDPE = DPE;
		cout << "DPE was set as" << fDPE << endl;
	}
	...
	void PMT_R11410::SetArea_sigma (Double_t Area_sigma) {
		...
		cout << "SPE area sigma was set as" << fArea_sigma << endl;
	}
	...
	void PMT_R11410::CalculateParams() {
		...
		switch (fMode) {			case kModeF1 :   
				...
				break;
			case kModeSpline :
				fAmpl_mean = SplineIntegral (fShape.spline, GetXmin (), GetXmax (), 1000) ;
				break;
			default:
				fAmpl_mean = 0;
				break;
	...
	Double_t PMT_R11410::SplineIntegral (TSpline* spline, Double_t xmin, Double_t xmax, Int_t nbins) {
		...
		for (int i=0; i<nbins; i++) {
			Integral += BinWidth * spline->Eval(xmin + i*BinWidth);
		}
		return Integral;
	}
...
if (fDebug) cout << "with amplitude = " << OnePulse.fAmpl/mV << " mV and time from '0' of OutWave =" << OnePulse.fTime/ns << " ns" << endl;
...
if (fDebug) cout << "photon didn't interact with PMT" << endl;
	...
	void PMT_R11410::Print(Option_t *option) const {
		...
		cout <<    "  DPE              =  " << fDPE*100 << " %";
		cout << " \t//Double Photoelectron Emission probability" << endl;
